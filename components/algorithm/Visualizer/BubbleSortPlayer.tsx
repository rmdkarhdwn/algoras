...
import { generateBubbleSortSteps } from "@/lib/algorithms/sorting/bubble";
import { useVisualizationStore } from "@/stores/visualizationStore";
import { useEffect, useState } from "react";

const DEFAULT_INPUT = [6, 3, 8, 1, 5, 2, 7, 4];

export function BubbleSortPlayer() {
  const { steps, currentStep, setSteps } = useVisualizationStore();
  const [inputValue, setInputValue] = useState(DEFAULT_INPUT.join(", "));
  const [error, setError] = useState("");

  // Initialize steps on mount
  useEffect(() => {
    setSteps(generateBubbleSortSteps(DEFAULT_INPUT));
  }, [setSteps]);
  ...
  function handleApply() {
    ...
    if (parsed.length < 2) {
      setError("2개 이상의 숫자를 입력하세요.");
      return;
    }
    if (parsed.length > 12) {
      setError("최대 12개까지 입력 가능합니다.");
      return;
    }
    ...
    <div className="flex flex-col gap-8">
      ...
      >
        {currentStepData ? (
          ...
          <div className="flex h-full items-center justify-center" style={{ color: "rgba(245,245,245,0.3)" }}>
            배열을 입력하고 시작하세요
          </div>
        )}
      </div>
      ...
      <div
        ...
        style={{ background: "rgba(20,20,20,0.6)" }}
      >
        <p className="mb-3 text-xs font-semibold uppercase tracking-widest" style={{ color: "rgba(245,245,245,0.35)" }}>
          배열 직접 입력
        </p>
